...
class WatchdogDaemon:
    ...
    def __init__(self, config: dict):
        ...
        from .daemon import _load_gateway_monitor, _load_ark_monitor, _load_qq_monitor, _load_activate_shadow, _load_start_shadow, _load_restart_ark, _load_notify
        
        _gw = _load_gateway_monitor(config)
        self._monitor_registry: dict[str, Monitor] = {
            "gateway.main_dead": _gw,
            "gateway.shadow_dead": _gw,
    ...
    def _match_rule(self, status: HealthStatus) -> str | None:
        """Find matching action for this status."""
        # FIXED: ONLY trigger actions if NOT healthy
        if status.healthy:
            return None
    ...
    async def _run_cycle(self):
        ...
        for monitor in active:
            ...
            # GLOBAL GUARD: If healthy, do nothing
            if status.healthy:
...
# Helper functions remain same as original...
...
def _load_ark_monitor(config: dict) -> "Monitor":
    ...
    return ArkMonitor(workspace=config.get("workspace", "~/.nanobot"))
