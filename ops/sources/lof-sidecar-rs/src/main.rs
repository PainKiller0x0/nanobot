...
function metric(k,v,cls=''){return `<div class="metricTile"><div class="k">${esc(k)}</div><div class="v ${cls}">${esc(v)}</div></div>`}
function metricNote(k,v,n,cls=''){return `<div class="metricTile"><div class="k">${esc(k)}</div><div class="v ${cls}">${esc(v)}</div><div class="tileNote">${esc(n)}</div></div>`}
...
function renderToday(){
  ...
  const brief=`<div class="briefGrid">
    ...
    <div class="briefBox"><div class="briefTitle">\u7cfb\u7edf</div><div class="briefMain">${mem.used_pct??'-'}%</div><div class="briefNote">\u670d\u52a1 ${side.healthy||0}/${side.total||0} \u6b63\u5e38\uff0cCPU \u538b\u529b ${loadFeeling(state.system?.loadavg||{},state.system?.cpu||{}).label}\uff0c\u5185\u5b58 ${mem.used_mb??'-'} / ${mem.total_mb??'-'} MB\u3002</div></div>
  </div>`;
  const focus=[];
